...
public class TargetMinimapOverlay extends Overlay {
	...
	public TargetMinimapOverlay(Client client, Configurations config, TargetController targetController) {
		this.client = client;
		this.config = config;
	...
	private static void drawMinimapIndicator(Graphics2D graphics, Point point, Color color, int diameter) {
		...
		try {
			...
			graphics.fillOval(point.getX() - radius, point.getY() - radius + 1, diameter, diameter);
