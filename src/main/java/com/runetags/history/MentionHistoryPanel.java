...
public class MentionHistoryPanel extends PluginPanel {
	...
	private static String contextText(
		...
		if (entry.getLocationName() != null && !entry.getLocationName().trim().isEmpty()) {
			if (text.length() > 0) {
				text.append(" \u2022 ");
			}

			text.append(entry.getLocationName());
		}
	...
	public void refreshFavoriteAppearance() {
		if (!SwingUtilities.isEventDispatchThread()) {
			SwingUtilities.invokeLater(this::refreshFavoriteAppearance);
			return;
		}

		for (FavoriteLabelBinding binding : favoriteLabels) {
			applyFavoriteAppearance(binding);
		}
	...
	private void applyFavoriteAppearance(
		...
		final boolean favorite = config != null
	...
	private void refreshAgeLabels() {
		if (!SwingUtilities.isEventDispatchThread()) {
			SwingUtilities.invokeLater(this::refreshAgeLabels);
			return;
		}

		for (AgeLabelBinding binding : ageLabels) {
			binding.label.setText(ageText(binding.timestamp));
		}
	}

	private static class FavoriteLabelBinding {
		private final JLabel label;
		private final String playerName;
		private final Color normalColor;

		private FavoriteLabelBinding(JLabel label, String playerName, Color normalColor) {
			this.label = label;
	...
	private static class AgeLabelBinding {
		...
		private AgeLabelBinding(JLabel label, Instant timestamp) {
			this.label = label;
